...
final class IvyDestinationSearch:NSObject,ObservableObject,MKLocalSearchCompleterDelegate {
    ...
    private var speechRequest:SFSpeechAudioBufferRecognitionRequest?; private var speechTask:SFSpeechRecognitionTask?; private var tapInstalled=false
    ...
    private static func normalizeSpokenAddress(_ text:String)->String {
        ...
        while i<words.count {
            ...
            if w=="linh" || w=="lẻ" { consumed=i+1;i+=1;continue }
            guard let n=d[w] else { break }
            saw=true
            let next=i+1<words.count ? words[i+1].lowercased():""
            if next=="trăm" { group += n*100;i+=2;consumed=i;continue }
            if next=="mươi" { group += n*10;i+=2;consumed=i;continue }
            group += n;i+=1;consumed=i
            if i<words.count { let unit=words[i].lowercased();if unit=="nghìn" || unit=="ngàn" { total += group*1000;group=0;i+=1;consumed=i } }
        }
        guard saw,consumed>0,total+group>0 else{return text}
        return ([String(total+group)]+Array(words.dropFirst(consumed))).joined(separator:" ")
    }
    func stopVoice(){if audioEngine.isRunning{audioEngine.stop()};if tapInstalled{audioEngine.inputNode.removeTap(onBus:0);tapInstalled=false};speechRequest?.endAudio();speechTask?.cancel();speechRequest=nil;speechTask=nil;isListening=false;try? AVAudioSession.sharedInstance().setActive(false,options:[.notifyOthersOnDeactivation])}
}
