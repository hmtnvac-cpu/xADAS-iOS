...
final class ADASWarningManager: NSObject, ObservableObject, AVAudioPlayerDelegate, AVSpeechSynthesizerDelegate {
    ...
    override init() {
        ...
        player = try? AVAudioPlayer(data: Self.makeBeepWAV()); player?.delegate = self; player?.prepareToPlay(); speechSynthesizer.delegate = self
    }
    ...
    func update(distance: LeadDistanceState, lane: LaneDepartureState, trafficSign: TrafficSignState, vehicleSpeedKPH: Double) {
        ...
        // Overspeed uses the live OSM/Valhalla road limit. It must not depend on
        // camera traffic-sign AI, which is intentionally paused in performance mode.
        let mapLimit = UserDefaults.standard.object(forKey: MapSpeedLimitProvider.currentLimitKey) as? Int
        if let limit = mapLimit, vehicleSpeedKPH >= Double(limit) + 5, now - lastOverspeedAlertAt >= 6 {
            alert(strong: true, message: "Cảnh báo quá tốc độ. Giới hạn \(limit) ki lô mét một giờ", key: "overspeed-map-\(limit)")
    ...
    private func updateLeadVehicleStart(distance: Double?, vehicleSpeedKPH: Double, now: TimeInterval) {
        ...
        guard ownCarStopped else { resetStoppedLead(); return }
        if let distance, distance >= 1.5, distance <= 35.0 {
            stoppedLeadLastSeenAt = now
            if stoppedLeadBaselineMeters == nil { stoppedLeadBaselineMeters = distance; stoppedLeadArmedAt = now; stoppedLeadMoveFrames = 0; wasStoppedWithLead = true; return }
        ...
        } else if wasStoppedWithLead, stoppedLeadArmedAt > 0, now - stoppedLeadArmedAt >= 1.2, now - stoppedLeadLastSeenAt >= 0.55, now - stoppedLeadLastSeenAt <= 2.5 { fireLeadVehicleStart(now: now) }
    }
    private func fireLeadVehicleStart(now: TimeInterval) {
        guard now - lastLeadStartAlertAt >= 5.0 else { return }
        alert(strong: false, message: "Xe phía trước đã di chuyển", key: "lead-vehicle-start", forceSpeech: true); lastLeadStartAlertAt = now; resetStoppedLead()
    }
    private func resetStoppedLead() { stoppedLeadBaselineMeters=nil; stoppedLeadArmedAt=0; stoppedLeadLastSeenAt=0; stoppedLeadMoveFrames=0; wasStoppedWithLead=false }
    ...
    private func announceNewTrafficSignIfNeeded(_ state: TrafficSignState) {
        ...
        lastTrafficSignUpdatedAt = state.updatedAt; guard sign != lastTrafficSign else { return }; lastTrafficSign = sign
        switch sign {
    ...
    private func alert(strong: Bool, message: String, key: String, forceSpeech: Bool = false) {
        activateWarningAudio(); let volume=warningVolume; player?.currentTime=0; player?.volume=strong ? min(volume*1.15,1):volume; player?.play()
        if vibrationEnabled { AudioServicesPlaySystemSound(kSystemSoundID_Vibrate) }
        guard audioMode == .allWarnings else { return }
        let now=ProcessInfo.processInfo.systemUptime; guard forceSpeech || key != lastSpokenKey || now-lastSpeechAt >= 5 else{return}
        if speechSynthesizer.isSpeaking { speechSynthesizer.stopSpeaking(at:.immediate) }
        let utterance=AVSpeechUtterance(string:message); utterance.voice=AVSpeechSynthesisVoice(language:"vi-VN"); utterance.rate=0.50; utterance.volume=volume; speechSynthesizer.speak(utterance); lastSpokenKey=key; lastSpeechAt=now
    }
    private func activateWarningAudio(){let session=AVAudioSession.sharedInstance();try? session.setCategory(.playback,mode:.default,options:[.mixWithOthers]);try? session.setActive(true)}
    private func releaseWarningAudioIfIdle(){guard player?.isPlaying != true,!speechSynthesizer.isSpeaking else{return};try? AVAudioSession.sharedInstance().setActive(false,options:[.notifyOthersOnDeactivation])}
    ...
    private static func makeBeepWAV()->Data {
        let sampleRate=22_050,duration=0.18,frequency=880.0,count=Int(Double(sampleRate)*duration);var pcm=Data(capacity:count*2)
        for index in 0..<count { let t=Double(index)/Double(sampleRate),fade=min(1.0,min(t/0.02,(duration-t)/0.04)),sample=sin(2 * .pi * frequency * t)*0.32*max(0,fade);var value=Int16(max(-1,min(1,sample))*Double(Int16.max)).littleEndian;withUnsafeBytes(of:&value){pcm.append(contentsOf:$0)} }
        var data=Data();func ascii(_ s:String){data.append(s.data(using:.ascii)!)};func u32(_ x:UInt32){var v=x.littleEndian;withUnsafeBytes(of:&v){data.append(contentsOf:$0)}};func u16(_ x:UInt16){var v=x.littleEndian;withUnsafeBytes(of:&v){data.append(contentsOf:$0)}}
        ascii("RIFF");u32(UInt32(36+pcm.count));ascii("WAVE");ascii("fmt ");u32(16);u16(1);u16(1);u32(UInt32(sampleRate));u32(UInt32(sampleRate*2));u16(2);u16(16);ascii("data");u32(UInt32(pcm.count));data.append(pcm);return data
    }
}
